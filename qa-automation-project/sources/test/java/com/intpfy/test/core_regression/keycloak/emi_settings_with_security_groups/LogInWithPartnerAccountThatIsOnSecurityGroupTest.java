...
public class LogInWithPartnerAccountThatIsOnSecurityGroupTest extends BaseWebTest implements KeycloakOrganizationTest {
    ...
    @Test(
            testName = "Log in with partner account that is on security group",
            description = "Keycloak user with Partner role and belonging to Security Group can Log in to EMI if Security Group added by Organization.",
            groups = {
                    ONE_USER,
                    EMI,
                    KEYCLOAK
    ...
    public void test() {
        ...
        // Sign in to EMI as user with Partner role and belonging to Security Group.

        KeycloakUser partnerWithSecurityGroup = getPartnerWithSecurityGroup();

        DashboardPage dashboardPage = keycloakPage.signIn(partnerWithSecurityGroup);
        ...
        dashboardPageVerifier.assertEmailEquals(partnerWithSecurityGroup.getEmail());
