...
public class LogInWithAccountFromSecurityGroupTest extends BaseWebTest implements KeycloakEventTest {
    ...
    @Test(
            ...
            description = "Keycloak user without Partner role but belonging to Security Group can not Log in to Event " +
                    "if it has Security Group and Allowlist not containing user email.",
            groups = {
                    ONE_USER,
                    EVENT,
                    KEYCLOAK
    ...
    public void test() {
        ...
        // Try to Sign in to Audience page as user without Partner role but belonging to Security Group.

        keycloakPage.signInAsAudience(getNotPartnerWithSecurityGroup());
