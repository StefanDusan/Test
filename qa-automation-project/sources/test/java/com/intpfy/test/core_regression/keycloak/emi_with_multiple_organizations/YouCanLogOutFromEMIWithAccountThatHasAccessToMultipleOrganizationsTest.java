...
public class YouCanLogOutFromEMIWithAccountThatHasAccessToMultipleOrganizationsTest extends BaseWebTest implements KeycloakTest {

    private final KeycloakAuthorizer authorizer = KeycloakAuthorizer.getInstance();

    @Test(
            testName = "Verify you can log out from EMI with account that has access to multiple organizations",
            description = "Keycloak user having multiple Keycloak Organizations can Log out from EMI.",
            groups = {
                    ONE_USER,
                    EMI,
                    KEYCLOAK
