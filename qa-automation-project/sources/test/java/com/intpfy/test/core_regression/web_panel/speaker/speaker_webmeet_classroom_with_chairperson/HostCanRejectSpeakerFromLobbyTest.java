...
public class HostCanRejectSpeakerFromLobbyTest extends BaseWebTest implements LobbyTest {
    ...
    @Test(
            testName = "Verify that Host can reject the Speaker from the Lobby Room",
            description = "Host can reject the Speaker from the Lobby Room.",
            groups = {
                    TWO_USERS,
                    EVENT,
                    SPEAKER,
    ...
    public void test() {
        ...
        hostVerifier.assertSpeakerLobbyActionsAvailable(speakerName);

        // Reject Speaker in Lobby.

        hostPage.rejectSpeakerInLobby(speakerName);
