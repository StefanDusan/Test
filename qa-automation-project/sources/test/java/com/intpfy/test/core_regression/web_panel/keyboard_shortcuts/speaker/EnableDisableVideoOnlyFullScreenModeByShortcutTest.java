...
public class EnableDisableVideoOnlyFullScreenModeByShortcutTest extends BaseWebTest implements EventTest {
    ...
    @Test(
            ...
            description = "Speaker can enable / disable 'Video only' Full screen mode by shortcut.",
            groups = {
                    TWO_USERS,
    ...
    public void test() {
        ...
        // Enable 'Video only' Full screen mode by shortcut.

        speakerPage.enableVideoOnlyFullscreenModeByShortcut();

        SpeakerVerifier speakerVerifier = new SpeakerVerifier(speakerPage);

        speakerVerifier.assertVideoOnlyFullscreenModeEnabled();

        // Disable 'Video only' Full screen mode by shortcut.

        speakerPage.disableVideoOnlyFullscreenModeByShortcut();
        speakerVerifier.assertVideoOnlyFullscreenModeDisabled();
